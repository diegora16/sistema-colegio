...
@section('breadcrumb', 'Configuración / Grados / Editar')
...
@section('content')
    ...
    <div class="max-w-md">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <form method="POST" action="{{ route('configuracion.grados.update', $grado) }}">
                @csrf
                @method('PUT')
                        ...
                        <select id="id_educativo"
                            ...
                            @foreach ($niveles as $nivel)
                                <option value="{{ $nivel->id }}"
                                    {{ old('id_educativo', $grado->id_educativo) == $nivel->id ? 'selected' : '' }}>
                                    {{ $nivel->nombre }}
